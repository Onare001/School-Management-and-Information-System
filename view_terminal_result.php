...
if (isset($_POST['class_terminal_result'])) {
    ...
    if (empty($class_id)) {
		...
		echo '<audio controls'.$autoplay.'hidden>
		 ...
		 $result = mysqli_query($conn, "SELECT score_id FROM score_info WHERE sch_id='$sch_id' AND class_id='$class_id' AND cat_id='$cat_id' AND term_id='$term_id' AND sid='$session_id'");
        if (mysqli_num_rows($result) != 0) {
			$row = mysqli_fetch_assoc($result);
			// Generate the URL
			$url = "load_terminal_result?cid=" . encrypt($class_id) . "&cat=" . encrypt($cat_id) . "&tid=" . encrypt($term_id) . "&ses_id=" . encrypt($session_id);
            /*header("location: $url");*/
...
} else if (isset($_POST['student_terminal_result'])) {
	...
	if (empty($username)) {
        ...
        if (mysqli_num_rows($result) != 0) {
			$row = mysqli_fetch_assoc($result);
			$url = "load_terminal_result?uid=". encrypt($uid) ."&cid=" . encrypt($class_id) . "&did=" . encrypt($cat_id) ."&tid=" . encrypt($term_id) . "&sid=" . encrypt($session_id) . "";
			...
			<section class="content" id="classes">
				<div class="card card-primary card-outline">
					<div class="card-header">
						<h3 class="card-title"><i class="fas fa-folder"></i> Terminal Result </h3>
						...
						<div class="row">
							...
							<div class="col-7 col-sm-9">
								<div class="tab-content" id="vert-tabs-tabContent">
									<center style="color:red;">Ensure that JavaScript pop up is enabled on this browser</center>
										...
										<form action="" method="post">
											<table align="center" border="0"  cellspacing="0" cellpadding="0" class="table" style="width:100%;float:left;">
												...
												<tr>
													<td align="right">
														<div class="col-md-12">&nbsp;
															<input name="class_terminal_result" type="submit" value="View Class Terminal Result" class="btn btn-primary"/>
														</div>
													</td>
												</tr>
											</table>
										...
										<form action="" method="post" autocomplete="off">
											<table align="center" border="0"  cellspacing="0" cellpadding="0" class="table" style="width:100%;">
												...
												<tr>
													<td align="right">
														<div class="col-md-12">&nbsp;
															<input name="student_terminal_result" type="submit" value="View Student Terminal Result" class="btn btn-primary"/>
														</div>
													</td>
												</tr>
											</table>	
