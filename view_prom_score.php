<?php $page_title = "View Promotion Score"; ?>
...
if (isset($_POST['submit'])) {
    ...
    if (empty($class_id)) {
        ...
        $sql = "SELECT * FROM cum_result WHERE sch_id='$sch_id' AND class_id='$class_id' AND cat_id='$cat_id' AND subj_id='$subj_id' AND sid='$session_id'"; 
		 $result = mysqli_query($conn, $sql);
        if (mysqli_num_rows($result) != 0) {
			$row = mysqli_fetch_assoc($result);
            header("location: prom_score?cid=" . encrypt($class_id) . "&did=" . encrypt($cat_id) . "&sid=" . encrypt($subj_id) ."&ses_id=" . encrypt($session_id) . "");
        } else {
            $msg = 'No Score Record Found for this Class in'.'&nbsp;'.getSession($session_id).'.';
			$msg_toastr = '<script>toastr.error("'.$msg.'")</script>';
			echo '<audio controls'.$autoplay.'hidden>
