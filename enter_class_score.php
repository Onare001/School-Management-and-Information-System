<?php $page_title = "Enter Class Score"; ?>
...
if (isset($_POST['submit'])) {
    ...
    if (empty($class_id)) {
        ...
        if (mysqli_num_rows($result) != 0) {
            ...
            header("location: enter_score?cid=" . encrypt($class_id) . "&did=" . encrypt($cat_id) . "&sid=" . encrypt($subj_id) . "&tid=" . encrypt($term_id) . "&sesid=" . encrypt($session_id));
        } else {
            $msg = 'Access Denied';
			$msg_toastr = '<script>toastr.error("'.$msg.'")</script>';
			echo '<audio controls'.$autoplay.'hidden>
...
<?php include('include/information.php');?>
				...
				<div class="card card-primary" id="selectbox" style="height:460px;">
					<div class="card-header">
						<h3 class="card-title"><i class="fa fa-percent"></i> Enter Score</h3>
					</div>
					<div class="card-body">
						<center style="margin-bottom:0px;"><?php if (isset($msg)) { echo $msg_toastr;} ?></center>
