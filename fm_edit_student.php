...
if(isset($_GET['uid'])) {
	...
	$sql = "SELECT * FROM sch_users JOIN stdnt_info ON sch_users.user_id=stdnt_info.user_id WHERE sch_users.user_id='$uid' AND stdnt_info.user_id='$uid'";
	...
	if (isset($_GET['uid']) && isset($_POST['submit'])){
		...
		$type_id = addslashes($_POST["type_id"]);	
	//Update info
	$update_info = "UPDATE `stdnt_info` SET  `rel_id`='$rel_id',`sex_id`='$sex_id',`dob`='$dob',`state_id`='$state_id',`lga`='$lga',`type_id`='$type_id',`admn_no`= '$admn_no',`address`='$address',`p_name`='$p_name',`relationship`='$relation',`parent_contact`='$parent_contact',`bld_grp`='$bld_id',`gtype`='$geno_id',`house_id`='$house_id',`club_id`='$club_id' WHERE `stdnt_info`.`user_id` = '$uid'";//`class_id`='$class_id',`cat_id` = '$cat_id', 
	$result = mysqli_query($conn,$update_info);
			
	//Update User
	$update_user = "UPDATE `sch_users` SET `first_name`= '$first_name',`last_name`= '$last_name' WHERE user_id = '$uid'";//,`username`= '$username'
	if (mysqli_query($conn,$update_user)){
			echo ('<script>alert("Updated Successfully")</script>');
		 } else {
		   echo "Error: " . $update_user . ":-" . mysqli_error($conn);
		 }
		 mysqli_close($conn);
