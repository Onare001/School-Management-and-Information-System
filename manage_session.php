							...
							<div class="card">
								...
								<div class="card-body">
									...
									<table id="example1" class="table table-bordered table-striped" style="width:70%">
										...
										<?php
										$counter = "0";
										$result = mysqli_query($conn,"SELECT * FROM session_info");
										while ($row = mysqli_fetch_array($result)){
										?>
										<tr> 
											<td align="center" width="5px"><?php echo ++$counter;?></td>
											<td><?php echo $row["session"];?></td>
											<td id="status-<?php echo $row['sid']; ?>" style="text-decoration:none;color:#007bff;"><?php echo ($row["status"]=='1') ? 'Active' : 'Inactive';?></td>
											<td align="center" width="5px"><?php echo ($row['status']=='1') ? '<a href="deactivate.php?sid='.$row['sid'].'">
											<img src="assets/img/tick.png" alt="active"></a>' : '<a href="activate.php?sid='.$row['sid'].'"><img src="assets/img/drop.png" alt="active"></a>'; ?></td>
											<td align="center" width="5px">
												<div class="custom-control custom-switch">
													<input type="checkbox" class="custom-control-input" id="customSwitch<?php echo $row['sid']; ?>" <?php echo ($row['status'] == '1') ? 'checked' : ''; ?> data-tid="<?php echo $row['sid'] ?>" data-state="<?php echo ($row['status'] == '1') ? '1' : '0'; ?>" data-table="session_info">
													<label class="custom-control-label" for="customSwitch<?php echo $row['sid']; ?>"></label>
												</div>
											</td>
										</tr>
				<?php } ?>	
